# Simple library to read data from Octopus Energy using their REST API
# Source code from:
...
class octoAPIClient(object):
    ...
    def get_simple(self, url):
        ...
        try:
            response = self.session.request(
                method="GET", url=url, auth=(self.api_key, ""), params={})
        except requests.RequestException as e:
            raise self.DataUnavailable("Network exception") from e
